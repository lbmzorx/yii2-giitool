...
column;
        ...
        foreach ($columns as $key=>$column){
            ...
            $comment=substr($column->comment,0,strpos($column->comment,'.code:'));

            $template.="| {$column->name} | {$allowNull} | {$column->dbType} | {$column->size} | {$column->precision} ".
                "| {$default} |  {$primaryKey} | {$autoIncrement}| {$comment} |\n";
